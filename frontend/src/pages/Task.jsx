...
function Task() {
    ...
    if (source.droppableId !== destination.droppableId) {
      ...
      setColumns((prev) => {
        return {
          ...
          [source.droppableId]: {
            ...sourceColumn,
            items: sourceItems.reduce((acc, curr, index) => {
              curr.index = index + 1;
              acc.push(curr);
              return acc;
            }, []),
          ...
          [destination.droppableId]: {
            ...destColumn,
            items: destItems.reduce((acc, curr, index) => {
              curr.index = index + 1;
              acc.push(curr);
              return acc;
            }, []),
  ...
  useEffect(() => {
    if (!isAddTaskModalOpen || isRenderChange) {
      todosApi
        ...
        .catch((error) => {
          toast.error("Get todo fails", error);
        });
    }
  }, [todoId, isAddTaskModalOpen, isRenderChange]);

  const updateTodo = (data) => {
    const todo = [];
    for (const key in data) {
      // todo.push({ items: data[key].items, name: data[key]?.name })
      for (const index in data[key].items) {
        data[key].items[index].stage = data[key].name;
        todo.push({
  ...
  const handleDelete = (e, taskId) => {
    ...
    tasksApi
      ...
      .catch((error) => {
        toast.error("handleDelete", error);
      });
  };
  ...
  return (
    <div className="px-12 py-6 w-full">
      <div className="flex items-center justify-between mb-6">
        <div className="text-xl text-gray-800 flex justify-start items-center space-x-2.5">
          <span className="">
            {title.slice(0, 25).toLocaleUpperCase()}
            {title.length > 25 && "..."}
          </span>
          <TodoDropdown id={todoId} navigate={navigate} />
        </div>
                                    ...
                                    >
                                      <div className="pb-2">
                                        ...
                                        <p className="text-xs text-slate-500 leading-4 -tracking-tight">
                                          {capitalizeFirstLetter(item.description.slice(0, 60))}
